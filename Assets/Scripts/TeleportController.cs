...
public class TeleportController : MonoBehaviour
{
    ...
    public void UpdateTeleportMarkerPosition()
    {
        ...
        {
            // 计算传送的目标位置
            teleportTarget = craftTransform.position + craftTransform.right * teleportDistance;

            // 检测从当前位置到传送目标之间是否有障碍物
            RaycastHit hit;
            if (Physics.Raycast(craftTransform.position, craftTransform.right, out hit, teleportDistance, obstacleLayer))
            {
    ...
    public void MoveTowardsTeleportTarget()
    {
        if (isTeleporting)
        {
            ...
            submachineController.SetVelocity(direction * teleportSpeed);
            // Debug.Log("执行传送");
        }
